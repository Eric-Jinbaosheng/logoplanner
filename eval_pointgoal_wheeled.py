...
parser.add_argument(
    "--min_goal_distance", type=float, default=25.0)
parser.add_argument(
...
def planning_thread(env, camera_intrinsic):
    ...
    while not stop_event.is_set():
        try:
            ...
            planner_output = pointgoal_step(goal, image, depth,port=args_cli.port)
            if len(planner_output) == 4:
                trajectory_points_camera, all_trajectories_camera, all_values_camera, _ = planner_output
            else:
                trajectory_points_camera, all_trajectories_camera, all_values_camera = planner_output
            trajectory_points_camera = np.asarray(trajectory_points_camera, dtype=np.float32)
            all_trajectories_camera = np.asarray(all_trajectories_camera, dtype=np.float32)
            all_values_camera = np.asarray(all_values_camera, dtype=np.float32)
            if (
                not np.isfinite(trajectory_points_camera).all()
                or not np.isfinite(all_trajectories_camera).all()
                or not np.isfinite(all_values_camera).all()
            ):
                raise ValueError("planner returned non-finite outputs")
            ...
            for idx in range(trajectory_points_camera.shape[0]):
                ...
                trajectory_points_world = np.asarray(trajectory_points_world, dtype=np.float32)
                if trajectory_points_world.ndim != 2 or trajectory_points_world.shape[0] < 2:
                    raise ValueError("planner returned too-short trajectory")
                if not np.isfinite(trajectory_points_world).all():
                    raise ValueError("planner returned non-finite world trajectory")
            ...
            for idx in range(all_trajectories_camera.shape[0]):
                ...
                for traj_camera in all_trajectories_camera[idx]:
                    ...
                    all_trajectories_world.append(np.asarray(traj_world, dtype=np.float32))
...
env_config.events.reset_pose.params = {"init_point_path":init_path, 
                                       'height_offset':0.1,
                                       'min_goal_distance': args_cli.min_goal_distance,
                                       'robot_visible': False,
                                       'light_enabled': False}
...
fps_writer = [imageio.get_writer(save_dir + "fps_%d.mp4"%i, fps=10) if args_cli.save_video else None for i in range(scene_config.num_envs)]
    ...
    with torch.inference_mode():
        ...
        if current_trajectory is not None:
            ...
            for i in range(args_cli.num_envs):
                ...
                try:
                    if not args_cli.save_video or vis_image is None:
                        continue
            ...
            if action_list:
                ...
                action = torch.zeros((args_cli.num_envs, 2), device="cuda:0")
                obs, rewards, dones, infos = env.step(action)
                print("MPC rejected trajectory, using zero action")
        else:
            action = torch.zeros((args_cli.num_envs, 2), device="cuda:0")
            obs, rewards, dones, infos = env.step(action)
        ...
        for i in range(args_cli.num_envs):
            if dones[i] == True:
                ...
                fps_writer[i] = imageio.get_writer(save_dir + "fps_%d.mp4"%episode_num, fps=10) if args_cli.save_video else None
