...
def pointnav_reset(env: ManagerBasedEnv, 
    ...
    if min_goal_distance > 0:
        distances = np.linalg.norm(sample_points[:, 0:2] - sample_points[:, 2:4], axis=1)
        sample_points = sample_points[distances > min_goal_distance]
        if sample_points.shape[0] == 0:
            raise ValueError(f"No pointgoal pairs with distance > {min_goal_distance} found in {init_point_path}")
...
def imagenav_reset(env: ManagerBasedEnv, 
    ...
    if min_goal_distance > 0:
        ...
        sample_points = sample_points[distances > min_goal_distance]
        if sample_points.shape[0] == 0:
            raise ValueError(f"No imagegoal pairs with distance > {min_goal_distance} found in {init_point_path}")
