...
from lact_decoder import LaCTDecoder
...
class GeometryModel(Pi3):
    def __init__(
        ...
        # Directly replace the inherited camera/local-point decoders with LaCT.
        self.point_decoder = LaCTDecoder(
        ...
        self.camera_decoder = LaCTDecoder(
            in_dim=2 * self.dec_embed_dim,
            dec_embed_dim=1024,
            dec_num_heads=16,
            out_dim=512,
    ...
    def forward(self, imgs, depths):
        ...
        with torch.amp.autocast(device_type='cuda', enabled=False):
            # local points
            point_hidden_fp32 = point_hidden[:, self.patch_start_idx:].float()
            world_point_hidden_fp32 = world_point_hidden.float()
            ret = self.point_head([point_hidden_fp32], (H, W)).reshape(B, N, H, W, -1)
            ...
            world_points = self.world_point_head([world_point_hidden_fp32], (H, W)).reshape(B, N, H, W, -1)
