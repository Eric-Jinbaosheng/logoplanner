...
class LoGoPlanner_Agent:
    def __init__(self,
        ...
        if missing_keys or unexpected_keys:
            print(
                f"[LoGoPlanner_Agent] loaded checkpoint with missing_keys={len(missing_keys)} "
    ...
    def step_pointgoal(self,goals,images,depths):
        with self._state_lock:
            ...
            for i in range(len(self.memory_queue)): # envs
                ...
                if len(self.depth_queue[i]) != memory_length:
                    raise RuntimeError(f"queue length mismatch for env {i}: rgb={memory_length}, depth={len(self.depth_queue[i])}")
